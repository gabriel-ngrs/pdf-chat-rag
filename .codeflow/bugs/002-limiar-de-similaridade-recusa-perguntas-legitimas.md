...
| `O que é a UFPB no documento?` | **recusou** | sim, página 2 |

E as mesmas informações são entregues quando a pergunta muda de forma:

| pergunta | resultado |
|---|---|
| `contato@yaitec.com` | responde certo |
| `UFPB` | responde certo |
...
"YAITEC" explicitamente — `"Qual é o e-mail de contato da YAITEC?"`,
`"Quais são os valores da YAITEC?"`. O nome da empresa aparece em quase todos os
...
1. **Acrescentar ao `eval/dataset.json` perguntas positivas sem a âncora** —
   ...
   contato?"`, `"O que é a UFPB?"`). São elas que revelam o piso real.
