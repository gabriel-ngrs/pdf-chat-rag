...
> **Nota de correção (2026-08-17, revisão 5):** três pontos corrigidos a partir do teste de ponta a ponta do owner, e nenhum deles muda escopo. (1) **FR-3** dizia "< 4 palavras" e o limiar subiu para **< 5**, com as formas contraídas (`dele, dela, nele, nela, disso, nisso`…) acrescentadas à lista de marcadores: com 4, "e a formação dele?" — quatro palavras, contração não reconhecida — ia crua ao retrieval e virava recusa falsa (BUG-001). O **AC-3** continua valendo, porque "qual o endereço da empresa?" tem exatamente cinco palavras. (2) **FR-8** dizia "dos chunks usados", e o que o código emite — desde sempre — são os chunks **consultados**, isto é, os que passaram do limiar. O texto passou a descrever o que existe, e a interface passou a rotular assim, em vez de prometer uma correspondência que o pipeline não apura (BUG-003, caminho escolhido pelo owner). (3) O `SIMILARITY_THRESHOLD` de **NFR-7** caiu de 0,625 para **0,561** após o dataset ganhar as perguntas sem o nome da empresa (BUG-002); a medição está em `backend/eval/README.md`.
...
- **FR-8** — Ao fim do streaming, um evento `citations` traz `{page_number, snippet, chunk_index, score}` dos chunks **consultados** — os que passaram de `SIMILARITY_THRESHOLD` e entraram no prompt. São os trechos em que a resposta *pode* ter se apoiado, não os que ela comprovadamente usou, e a interface os rotula assim. O `snippet` tem no máximo 240 caracteres, recortado em fronteira de palavra **no servidor**.
...
- **AC-10** (FR-8) — *Dado* uma resposta fundamentada, *quando* o streaming termina, *então* chega `citations` com `page_number`, `snippet` (≤ 240 chars, sem cortar palavra), `chunk_index` e `score`, uma entrada por chunk que passou do limiar; *e* a interface os apresenta como trechos **consultados**, sem prometer que cada um sustenta a resposta.
...
- **Passos:**
  1. **`should_condense(history, question) -> bool`** — verdadeiro se há histórico **e** (a pergunta tem menos de 5 palavras **ou** contém um dos marcadores anafóricos de FR-3). Lista de marcadores como constante nomeada e documentada.
...
- **Depende de:** `A.5`
- **Contexto que o agente precisa:** esta fase **só começa com `A.1`–`A.6` e `B.1`–`B.4` fechadas**. *(Correção de 2026-08-17: a pré-condição dizia `B.1`–`B.5`. Era a ordem errada. Esta fase altera `db/002_conversations.sql` e exige reingestão, enquanto a `B.5` **congela** o estado do app — vídeo, GIF, resultados do eval colados e ensaio de clone limpo. Rodar a `A.7` depois da `B.5` invalidaria a demonstração recém-gravada. O owner decidiu executá-la, e ela passa a vir antes.)* Ela altera `db/002_conversations.sql`, o que exige `make down` e reingestão — avalie se vale o custo àquela altura; se não valer, não execute e registre no README como próximo passo. A config `portuguese` do `to_tsvector` vem de fábrica na imagem `pgvector/pgvector:pg16`.
...
- **Escopo travado / violações BLOQUEANTES:** a fusão vive em `core/`, sem tocar banco; **não substituir** a busca densa — fundir; não executar esta fase antes de `A.1`–`A.6` e `B.1`–`B.4` estarem concluídas; **não iniciar a `B.5` antes desta fase fechar**, porque a `B.5` congela o estado do app e esta o altera.
...
- **Depende de:** `B.4`, `A.5`
- **Pré-condição adicional do owner (2026-08-17):** a `A.7` foi promovida a **anterior** a esta fase — o owner decidiu executá-la, e ela altera o schema, então precisa fechar antes de qualquer gravação. Além dela, esta fase **só começa depois de o owner testar o sistema de ponta a ponta, deixá-lo consistente e corrigir os bugs que aparecerem**. O motivo é a natureza dos entregáveis: o vídeo, o GIF e o ensaio de clone limpo **congelam** o estado do app no momento em que são gravados, e o convite ao colaborador expõe o repositório. Gravar antes de o sistema estar estável significa regravar — ou, pior, entregar uma demonstração que não corresponde ao código. Bug encontrado nessa bateria volta como `/bugfix` ou como rework da fase dona, **não** é consertado dentro da `B.5`. O executor não inicia esta fase por conta própria: aguarda o aval explícito do owner.
